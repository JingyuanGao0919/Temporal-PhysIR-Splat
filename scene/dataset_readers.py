...
def _timestamp_seconds_from_name(name):
    ...
    minute = int(code[2:4])
    second = int(code[4:6])
    millisecond = int(code[6:9])
    return hour * 3600.0 + minute * 60.0 + second + millisecond / 1000.0


def _normalise_camera_timestamps(cam_infos):
    if not cam_infos:
        return cam_infos

    raw_times = []
    for idx, cam in enumerate(cam_infos):
        timestamp = _timestamp_seconds_from_name(cam.image_name)
        raw_times.append(float(idx) if timestamp is None else timestamp)

    unwrapped = []
    offset = 0.0
    last = raw_times[0]
    for timestamp in raw_times:
        if timestamp + 1e-9 < last:
            offset += 86400.0
        unwrapped.append(timestamp + offset)
        last = timestamp

    span = max(unwrapped[-1] - unwrapped[0], 1e-6)
    return [
        cam._replace(timestamp=(unwrapped[idx] - unwrapped[0]) / span)
        for idx, cam in enumerate(cam_infos)
    ]


def _resolve_colmap_image_path(images_folder, image_name):
    candidates = [
        os.path.join(images_folder, os.path.basename(image_name)),
    ]

    stem, ext = os.path.splitext(os.path.basename(image_name))
    if "_" in stem:
        candidates.append(os.path.join(images_folder, stem.split("_", 1)[1] + ext))
    candidates.append(os.path.join(images_folder, image_name.replace("color", "red")))
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        image_path = _resolve_colmap_image_path(images_folder, extr.name)
        image_name = os.path.basename(image_path).split(".")[0]
        image = Image.open(image_path)
        fid = idx / max(num_frames - 1, 1)
        timestamp = _timestamp_seconds_from_name(image_name)
        timestamp = fid if timestamp is None else timestamp
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    cam_infos = sorted(cam_infos_unsorted.copy(), key=lambda x: _image_sort_key(x.image_name))
    cam_infos = _normalise_camera_timestamps(cam_infos)
    cam_infos = [
        c._replace(fid=idx / max(len(cam_infos) - 1, 1))
        for idx, c in enumerate(cam_infos)
    ]

    if eval:
        if os.path.basename(path) == "thermal":
            part = 4
            total_count = len(cam_infos)
            ...
            for i in range(part):
                ...
                middle_start = start_idx + (part_length - test_count) // 2
                middle_end = middle_start + test_count
                test_indices.update(range(middle_start, middle_end))
        else:
